...
export async function getPricingRecommendations(configOverrides: Partial<PricingEngineConfig> = {}, limit = 200): Promise<{ config: PricingEngineConfig; recommendations: PricingRecommendation[] }> {
  ...
  const products = await prisma.product.findMany({
    where: { isActive: true },
    include: {
      markets: { where: { isActive: true }, include: { country: true }, orderBy: { country: { code: 'asc' } } },
      matches: {
        where: { matchStatus: MatchStatus.CERTAIN },
  ...
  const recommendations = products.flatMap<PricingRecommendation>((product) => product.markets.map((market) => {
    const marketCurrency = market.currency || market.country.currency
    const ownPrice = numeric(market.ownPrice ?? (product.currency === marketCurrency ? product.ownPrice : null))
    const offers = product.matches
      .map((match) => match.competitorOffer)
      .filter((offer) => offer.isActive && offer.competitor.isActive && offer.competitor.countryId === market.countryId && offer.normalizedPrice !== null)
      .filter((offer) => !config.onlyInStock || isStockRelevant(offer.stockStatus))
      .filter((offer) => (offer.currency || marketCurrency).toUpperCase() === marketCurrency.toUpperCase())
    ...
    if (!ownPrice || prices.length === 0) {
      return {
        ...
        action: 'NO_DATA',
        marketPosition: null,
        reason: ownPrice ? `Nog geen bruikbare, goedgekeurde concurrentieprijs voor ${market.country.name}.` : `Eigen verkoopprijs voor ${market.country.name} ontbreekt.`,
        guardrailNotes: ['Kostprijs en minimum marge zijn nog niet als productveld beschikbaar.'],
      }
    }
    ...
    return {
      ...base,
      recommendedPrice: roundedTarget,
      changePct,
      action,
      marketPosition,
      reason: action === 'KEEP' ? `Geen actie nodig in ${market.country.name}, verschil blijft binnen ${config.minimumSignalPct.toFixed(1)}%.` : `Advies voor ${market.country.name} op basis van ${strategyNames[config.strategy]} met maximaal ${config.maxChangePct.toFixed(1)}% wijziging per besluit.`,
      guardrailNotes: [`Alleen ${prices.length} goedgekeurde ${config.onlyInStock ? 'en beschikbare ' : ''}concurrentieprijzen uit ${market.country.name} zijn gebruikt.`, 'Automatisch publiceren is bewust niet actief zolang kostprijs, margegrenzen en een writeback koppeling ontbreken.'],
    }
  }))
