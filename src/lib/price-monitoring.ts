...
function typeNames(value: unknown) { if (typeof value === 'string') return [value]; if (Array.isArray(value)) return value.filter((item): item is string => typeof item === 'string'); return [] }
function walkJson(value: unknown, records: JsonRecord[] = []): JsonRecord[] { if (Array.isArray(value)) { value.forEach((item) => walkJson(item, records)); return records } if (!value || typeof value !== 'object') return records; const record = value as JsonRecord; records.push(record); Object.values(record).forEach((item) => { if (item && typeof item === 'object') walkJson(item, records) }); return records }
function availabilityLabel(value: unknown) { const normalized = String(value ?? '').toLowerCase(); if (!normalized) return null; if (normalized.includes('instock') || normalized.includes('in_stock') || normalized.includes('op voorraad')) return 'Op voorraad'; if (normalized.includes('outofstock') || normalized.includes('out_of_stock') || normalized.includes('niet op voorraad')) return 'Niet op voorraad'; if (normalized.includes('preorder') || normalized.includes('pre-order')) return 'Pre-order'; if (normalized.includes('backorder')) return 'Nabestelling'; if (normalized.includes('limited') || normalized.includes('beperkt')) return 'Beperkt'; return text(value) }

function extractJsonLd(html: string): ExtractedOffer | null {
  const scripts = [...html.matchAll(/<script\b[^>]*type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)]
  for (const script of scripts) {
    const raw = script[1]?.trim(); if (!raw) continue
    try {
      ...
      if (!price) continue
      return { price, currency: text(candidate.priceCurrency), stockStatus: availabilityLabel(candidate.availability), productTitle: text(product?.name), sku: text(product?.sku ?? candidate.sku), ean: text(product?.gtin13 ?? product?.gtin14 ?? product?.gtin ?? product?.ean), packagingQty: null, method: 'JSON_LD' }
    } catch { continue }
  }
  return null
}
function attribute(tag: string, name: string) { const quoted = tag.match(new RegExp(`${name}\\s*=\\s*["']([^"']+)["']`, 'i')); if (quoted?.[1]) return quoted[1]; const bare = tag.match(new RegExp(`${name}\\s*=\\s*([^\\s>]+)`, 'i')); return bare?.[1] ?? null }
function extractMeta(html: string): ExtractedOffer | null {
  const meta = [...html.matchAll(/<meta\b[^>]*>/gi)].map((match) => match[0])
  const values = new Map<string, string>()
  for (const tag of meta) { const key = (attribute(tag, 'property') ?? attribute(tag, 'name') ?? attribute(tag, 'itemprop'))?.toLowerCase(); const content = attribute(tag, 'content'); if (key && content) values.set(key, content) }
  const price = ['product:price:amount', 'og:price:amount', 'price', 'product.price.amount'].map((key) => parseLocalizedPrice(values.get(key))).find((value) => value !== null) ?? null
  if (!price) return null
  return { price, currency: ['product:price:currency', 'og:price:currency', 'pricecurrency'].map((key) => values.get(key)).find(Boolean) ?? null, stockStatus: availabilityLabel(values.get('product:availability') ?? values.get('availability')), productTitle: values.get('og:title') ?? null, sku: values.get('sku') ?? null, ean: values.get('gtin13') ?? values.get('gtin') ?? null, packagingQty: null, method: 'META' }
}
function extractHtmlFallback(html: string): ExtractedOffer | null {
  const compact = html.replace(/\s+/g, ' ')
  const patterns: Array<{ regex: RegExp; currency: string }> = [{ regex: /€\s*([0-9][0-9.,\s]{0,14})/i, currency: 'EUR' }, { regex: /([0-9][0-9.,\s]{0,14})\s*€/i, currency: 'EUR' }, { regex: /£\s*([0-9][0-9.,\s]{0,14})/i, currency: 'GBP' }, { regex: /(?:DKK|kr\.?)[\s:]?([0-9][0-9.,\s]{0,14})/i, currency: 'DKK' }]
  for (const pattern of patterns) {
    const match = compact.match(pattern.regex); const price = parseLocalizedPrice(match?.[1]); if (!price) continue
    const title = compact.match(/<title[^>]*>([^<]{2,180})<\/title>/i)?.[1]?.trim() ?? null
    const stockStatus = /niet op voorraad|out of stock|sold out/i.test(compact) ? 'Niet op voorraad' : /op voorraad|in stock|available/i.test(compact) ? 'Op voorraad' : null
...
export async function runDuePriceChecks({ limit = 40, competitorOfferId, productId, force = false }: { limit?: number; competitorOfferId?: string; productId?: string; force?: boolean } = {}) {
  const cappedLimit = Math.min(Math.max(limit, 1), 200)
  const offers = await prisma.competitorOffer.findMany({ where: { id: competitorOfferId, isActive: true, competitor: { isActive: true }, productMatch: productId ? { productId, matchStatus: { in: [MatchStatus.CERTAIN, MatchStatus.REVIEW] } } : { matchStatus: { in: [MatchStatus.CERTAIN, MatchStatus.REVIEW] } } }, include: { competitor: true }, orderBy: { lastCheckedAt: 'asc' }, take: Math.min(cappedLimit * 5, 500) })
  const now = Date.now()
  const due = offers.filter((offer) => force || competitorOfferId || !offer.lastCheckedAt || now - offer.lastCheckedAt.getTime() >= offer.competitor.checkFrequencyHours * 60 * 60 * 1000).slice(0, cappedLimit)
  const results = []
  for (const offer of due) results.push(await runPriceCheck(offer.id))
  return { requested: cappedLimit, due: due.length, successful: results.filter((result) => result.success).length, failed: results.filter((result) => !result.success).length, results }
}
